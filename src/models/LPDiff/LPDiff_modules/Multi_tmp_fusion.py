...
class MTA(nn.Module):
    ...
    def forward(self, f1, f2, f3, f4, f5):
        ...
        f4_multi_scale = self.encoder(f4)
        f5_multi_scale = self.encoder(f5)
        ...
        crossatt_3 = self.crossatt(f3_multi_scale, f4_multi_scale) + f4_multi_scale
        crossatt_4 = self.crossatt(f4_multi_scale, f5_multi_scale) + f5_multi_scale
        GCA1 = self.GCA(crossatt_1)
        GCA2 = self.GCA(crossatt_2)
        GCA3 = self.GCA(crossatt_3)
        GCA4 = self.GCA(crossatt_4)
        channel_output_1 = self.IntraframeAtt(GCA1, GCA2)
        channel_output_2 = self.IntraframeAtt(GCA3, GCA4)
        final_output = self.IntraframeAtt(channel_output_1, channel_output_2)
...
if __name__ == '__main__':
    ...
    # model = model.cuda()
    t1 = torch.randn(5, 3, 32, 128)
    t2 = torch.randn(5, 3, 32, 128)
    t3 = torch.randn(5, 3, 32, 128)
    t4 = torch.randn(5, 3, 32, 128)
    t5 = torch.randn(5, 3, 32, 128)
    # t1 = t1.cuda()
    # t2 = t2.cuda()
    # t3 = t3.cuda()
    output = model(t1, t2, t3, t4, t5)
    print(output.shape)
